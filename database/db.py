...
def save_farm(db_path: str, farm: dict) -> int:
    ...
    cur.execute("""
        INSERT INTO farms
        (district, tehsil, area_ha, crop, sowing_date, soil_type,
    ...
    """, (
        ...
        farm["water_source"], farm["canal_water_m3"], datetime.now().isoformat(timespec="seconds")
    ))
...
def save_water_account(db_path: str, farm: dict, result: dict):
    conn = get_connection(db_path)
    cur = conn.cursor()

    cur.execute("""
        SELECT farm_id FROM farms
        ORDER BY farm_id DESC LIMIT 1
    """)
    row = cur.fetchone()
    ...
    cur.execute("""
        ...
        (farm_id, eto_mm, kc, etc_mm, effective_rain_mm,
         soil_water_contribution_mm, net_irrigation_mm,
         application_efficiency, gross_irrigation_mm, gross_volume_m3,
    ...
    """, (
        ...
        result["groundwater_dependency_pct"],
        datetime.now().isoformat(timespec="seconds"),
    ))
    conn.commit()
    conn.close()

def fetch_recent_accounts(db_path: str, limit: int = 10):
    conn = get_connection(db_path)
    cur = conn.cursor()
    cur.execute("""
        SELECT account_id, eto_mm, etc_mm, gross_volume_m3,
               groundwater_m3, groundwater_dependency_pct, created_at
        FROM water_accounts
